...
const PRODUCTS = [
  ...
  { id: "p4", title: "Habanero (250g)", price: 1200, category: "peppers", source: "open_field", image: "assets/habanero.jpg", lot: "LOT20251106", harvest: "2025-11-01" },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sourceLabel = s => s === 'greenhouse' ? 'Greenhouse' : 'Open field';
  ...
  if (document.getElementById('products')) {
    ...
    function render(list) {
      container.innerHTML = '';
      if (list.length === 0) { container.innerHTML = '<p>No produce matches that search right now.</p>'; return; }
      list.forEach(p => {
      ...
      container.querySelectorAll('.add').forEach(b => b.addEventListener('click', e => {
        ...
        if (exists) exists.qty += 1; else cart.push({ id: prod.id, title: prod.title, price: prod.price, qty: 1 });
        saveCart(cart);
      }));
    }
    render(PRODUCTS);
    ...
    function applyFilters() {
      const gh = ghFilter.checked, of = ofFilter.checked, q = search.value.toLowerCase();
      let list = PRODUCTS.filter(p => p.title.toLowerCase().includes(q) || p.category.includes(q));
  ...
  if (cartBtn) cartBtn.addEventListener('click', () => { cartModal.setAttribute('aria-hidden', 'false'); renderCart(); });
  if (closeCart) closeCart.addEventListener('click', () => cartModal.setAttribute('aria-hidden', 'true'));
  ...
  function renderCart() {
    ...
    node.innerHTML = '';
    let total = 0;
    if (items.length === 0) node.innerHTML = '<p>Your cart is empty.</p>';
    items.forEach(it => {
  ...
  function changeQty(id, delta) {
    ...
    if (!it) return;
    it.qty += delta;
    if (it.qty <= 0) { const idx = cart.findIndex(x => x.id === id); cart.splice(idx, 1); }
  ...
  if (lookup) {
    lookup.addEventListener('click', () => {
      ...
      if (!prod) { res.innerHTML = '<div class="result-note">Lot not found. Check the code on your pack and try again.</div>'; return; }
      res.innerHTML = `
    ...
    if (params.has('lot')) { document.getElementById('lot-input').value = params.get('lot'); lookup.click(); }
  }
